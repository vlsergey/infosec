...
\begin{enumerate}
    \item $C \rightarrow S$:
        \begin{enumerate}
            \item ClientHello: \begin{inparaenum}[1)] \item версия протокола, поддерживаемая клиентом, \item одноразовая метка $N_C$\index{одноразовая метка}, \item поддерживаемые алгоритмы шифрования, кода аутентификации сообщений, хэширования, ЭП и сжатия в порядке убывания приоритета.\end{inparaenum}
    ...
    \item $C \leftarrow S$:
        \begin{enumerate}
            ...
            \item Электронная подпись к Server Key Exchange на ключе сертификата сервера для аутентификации сервера клиенту.
    ...
    \item $C \rightarrow S$:
        \begin{enumerate}
            ...
            \item Электронная подпись к Client Key Exchange на ключе сертификата клиента для аутентификации клиента серверу (если клиент использует сертификат).
