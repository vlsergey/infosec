...
\begin{enumerate}
    ...
    \item $C \rightarrow S$:
        \begin{enumerate}
            ...
            \item Client Key Exchange Message: информация для создания предварительного общего секрета $premaster$ длиной 48 байтов одним из следующих способов: ~ 1) обменом по протоколу Диффи~---~Хеллмана\index{протокол!Диффи~---~Хеллмана} с сервером (клиент отсылает $g^b$, в результате обе стороны вычисляют ключ $premaster = g^{ab}$), ~ 2) шифрованием ключа, выбранного клиентом, на открытом ключе из сертификата сервера, ~ 3) другим способом.
