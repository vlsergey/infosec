...
\begin{enumerate}
    \item $C \rightarrow S$:
        \begin{enumerate}
            \item ClientHello: \begin{inparaenum}[1)] \item версия протокола, поддерживаемая клиентом, \item одноразовая метка $N_C$\index{одноразовая метка}, \item поддерживаемые алгоритмы шифрования, кода аутентификации сообщений, хэширования, ЭЦП и сжатия в порядке убывания приоритета.\end{inparaenum}
                  Также возможно использование расширений сообщения ClientHello. Например, при использовании расширения SNI (Server Name Indication) возможно указать имя сервера (hostname), которое используется в случае нескольких виртуальных веб-серверов с разными именами, но одним IP адресом.
        \end{enumerate}
    ...
    \item $C \leftarrow S$:
        \begin{enumerate}
            ...
            \item Server Certificate: сертификат X.509v3 сервера.
            \item Server Key Exchange: информация для создания предварительного общего секрета $premaster$ длиной 48 байтов в виде: \begin{inparaenum}[1)] \item либо обмена по протоколу Диффи---Хеллмана с клиентом (сервер отсылает $(g, g^a)$), \item либо по другому алгоритму с открытым ключом, \item либо разрешения клиенту выбрать ключ.\end{inparaenum}
            \item Электронно-цифровая подпись к Server Key Exchange на ключе сертификата сервера для аутентификации сервера клиенту.
    ...
    \item $C \rightarrow S$:
        \begin{enumerate}
            ...
            \item Client Key Exchange: информация для создания предварительного общего секрета $premaster$ длиной 48 байтов в виде: \begin{inparaenum}[1)] \item либо обмена по протоколу Диффи---Хеллмана с сервером (клиент отсылает $g^b$, в результате обе стороны вычисляют ключ $premaster = g^{ab}$), \item либо по другому алгоритму, \item либо ключа, выбранного клиентом и зашифрованного на открытом ключе из сертификата сервера.\end{inparaenum}
            \item Электронно-цифровая подпись к Client Key Exchange на ключе сертификата клиента для аутентификации клиента серверу (если клиент использует сертификат).
